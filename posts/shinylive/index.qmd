...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel("Forecasting via the Browser"),
  sidebarLayout(
...
In theory, there is not that much to it so long as the app works with `webR`.  So let's compile it up and see what happens.  There is an `app.R` file in `myapp` and that becomes the `docs` directory when transformed by shiny live.

```
shinylive::export(appdir = "myapp", outputdir = "docs")
```

If it works, the `docs` directory has a [forecasting app](docs/index.html).
